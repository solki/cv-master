  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500 mt-1">System configuration and status</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <StatusCard title="System Health" status={healthError ? "error" : health?.status === "ok" ? "healthy" : "error"}>
          <p className="text-sm text-slate-600">Environment: {health?.environment || "N/A"}</p>
          <p className="text-sm text-slate-600">Database: {health?.database?.status || "N/A"}</p>
        </StatusCard>
        <StatusCard title="LLM Provider" status={llmError ? "error" : llmHealth?.llm?.configured ? "configured" : "not configured"}>
          <p className="text-sm text-slate-600">Provider: {llmHealth?.llm?.provider || "N/A"}</p>
          <p className="text-sm text-slate-600">Search (Tavily): {llmHealth?.search?.configured ? "configured" : "not configured"}</p>
        </StatusCard>
      </div>

      <div className="bg-slate-900 rounded-lg shadow-sm border border-slate-700 p-6 space-y-3">
        <h2 className="font-semibold">Configuration</h2>
        <p className="text-sm text-slate-500">
          LLM provider, API keys, and search configuration are managed via environment variables.
          See <code className="text-xs bg-slate-800 px-1 rounded">.env.example</code> for all available options.
        </p>
        <p className="text-sm text-slate-500">
          Set <code className="text-xs bg-slate-800 px-1 rounded">LLM_PROVIDER</code> to one of:
